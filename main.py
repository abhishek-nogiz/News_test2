...
from news_agent_working.services import PublisherService
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # ── NEW: Internal Linking Indexing flags ──
    parser.add_argument(
        "--index",
        action="store_true",
        help="Run the indexing service only (crawl sitemap → embed → store). Does NOT run the pipeline.",
    )
    parser.add_argument(
        "--sitemap-url",
        default=None,
        help="Override sitemap URL for indexing (e.g. https://www.peoplenewstime.com/sitemap.xml)",
    )
    parser.add_argument(
        "--tenant-id",
        default=None,
        help="Tenant ID for multi-tenant setups (defaults to config value or empty string)",
    )
    parser.add_argument(
        "--vector-store",
        choices=["json", "pgvector"],
        default=None,
        help="Vector store backend: json (dev) or pgvector (production)",
    )
    # ── END NEW ──
...
def main() -> int:
    ...
    config = replace(
        ...
        # ── NEW: Internal linking overrides ──
        sitemap_url=args.sitemap_url or config.sitemap_url,
        tenant_id=args.tenant_id or config.tenant_id,
        vector_store_type=args.vector_store or config.vector_store_type,
    )

    # ── WordPress auth check (unchanged) ──
    ...
    # ── NEW: --index mode (runs only the indexing service, not the pipeline) ──
    if args.index:
        tenant_id = config.tenant_id or "_default"
        result = pipeline.indexing_service.index_tenant(tenant_id)
        print(json.dumps(dict(result), indent=2))
        return 0
    # ── END NEW ──

    # ── Normal pipeline run (UNCHANGED from original) ──
